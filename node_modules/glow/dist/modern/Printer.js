...
class Printer {
  ...
  async _printExtra(message) {
    ...
    for (let messagePart of message.message) {
      res.push((await this._printMessagePart(messagePart, {
        linesAbove: 1,
        linesBelow: 1
      })));
    }
  ...
  async _printMessagePart(messagePart, opts = {}) {
    ...
    } else if (messagePart.type === 'Comment' || messagePart.type === 'Blame' && messagePart.context === null) {
  ...
  async _printBlameMessagePart(messagePart, opts = {}) {
    return await this._printLocation(messagePart.loc, (0, _extends3.default)({
  ...
  async _printFileLocation(loc, opts = {}) {
    ...
    let fileContents = await (0, _fs.readFileWithCache)(new _map2.default(), loc.source);
    let babelLoc = (0, _locations.toBabelSourceLocation)(loc);
    let codeFrame = (0, _codeFrame.codeFrameColumns)(fileContents, babelLoc, (0, _extends3.default)({
      highlightCode: true
    }, opts));
    return `${link}\n${codeFrame}`;
  }
