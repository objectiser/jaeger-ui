...
class Logger {
  ...
  write(stream, kind, message, opts = {}) {
    ...
    let fullPrefix = opts.emoji && !this.env.interface ? `${opts.emoji}  ${prefix}` : prefix;

    str = opts.style ? opts.style(str) : str;
    str = fullPrefix == '' ? str : str.split('\n').map(line => `${fullPrefix}${line}`).join('\n');
  ...
  title(message, opts = {}) {
    this.write('stderr', 'info', (0, _Lang.langMessageToString)(message), toFormatOptions(opts, {
  ...
  debug(message, opts = {}) {
    this.write('stderr', 'debug', (0, _Lang.langMessageToString)(message), toFormatOptions(opts, {
      prefix: _chalk2.default.magenta('debug')
    }));
  }

  warn(message, opts = {}) {
    this.write('stderr', 'warn', (0, _Lang.langMessageToString)(message), toFormatOptions(opts, {
      prefix: _chalk2.default.yellow('warn')
    }));
  }

  error(message, opts = {}) {
    this.write('stderr', 'error', (0, _Lang.langMessageToString)(message), toFormatOptions(opts, {
      prefix: _chalk2.default.yellow('red')
    }));
  }

  line() {
    this.write('stderr', 'output', '\n' + _chalk2.default.gray('─'.repeat(_constants.STDOUT_WIDTH)) + '\n');
  }
}
