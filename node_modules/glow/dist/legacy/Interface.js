...
var Interface = exports.Interface = function (_EventEmitter) {
  ...
  function Interface(opts) {
    ...
    _this.screen.on('keypress', function (char, meta) {
      ...
      if (meta.name === 'enter' || meta.name === 'return' || meta.ctrl === true) {
        return;
  ...
  Interface.prototype._setCurrentIndex = function _setCurrentIndex(index) {
    ...
    var sections = this._results.map(function (result, index) {
      return result.message.split('\n').map(function (str) {
        if (_this2._currentIndex === index) {
          return startPadding + str;
        } else {
          return startPadding + _chalk2.default.gray((0, _stripAnsi2.default)(str));
        }
      }).join('\n');
    }).map(function (str) {
    ...
    var lastSectionHeight = sections[sections.length - 1].split('\n').length - 1;

    var topPad = 4;
    var bottomPad = Math.max(_constants.STDOUT_HEIGHT - lastSectionHeight - topPad, topPad);
    ...
    var output = '\n'.repeat(topPad - sectionPad) + sections.join('') + '\n'.repeat(bottomPad);
